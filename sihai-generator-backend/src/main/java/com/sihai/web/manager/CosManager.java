...
import com.sihai.web.config.CosClientConfig;
...
public class CosManager {
    ...
    @PostConstruct
    public void init() {
        // 多线程并发上传下载：自定义线程池大小，建议在客户端与 COS 网络充足（例如使用腾讯云的 CVM，同地域上传 COS）的情况下，设置成16或32即可，可较充分的利用网络资源
    ...
    public Download download(String key, String localFilePath) throws InterruptedException {
        Download download = transferManager.download(new GetObjectRequest(cosClientConfig.getBucket(), key),
                new File(localFilePath));
     ...
     * 批量删除对象
     */
    public DeleteObjectsResult deleteObjects(List<String> keyList) throws CosClientException, CosServiceException, MultiObjectDeleteException {
     ...
     * 删除目录
     */
    public void deleteDir(String delPrefix) throws CosClientException, CosServiceException, MultiObjectDeleteException {
